...
def line_to_token_ids(line, vocabulary, tokenizer=None, normalize_digits=False):
    line = line.split('\t')
    res_str = ''
    for sen in line:
        ids = sentence_to_token_ids(sen.strip(), vocabulary, tokenizer, normalize_digits)
        if len(ids) > 0:
            str_ids = " ".join([str(tok) for tok in ids])
            if res_str == '':
                res_str = str_ids
            else:
                res_str += '\t' + str_ids
    ...
    with open(data_path, mode="r") as data_file:
        with open(target_path, mode="w") as tokens_file:
            ...
            for line in data_file:
                ...
                line = line.strip()
                token_str = line_to_token_ids(line, vocab, tokenizer,
                                                  normalize_digits)
